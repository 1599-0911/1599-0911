var SITE_SHOP_MYPAGE = function(){
  ...
  var trackingParcel = function(code, is_c3 = false){
	...
	const omsOrderConfirm = async function(orderCode, orderSectionCode){
		...
		if (!window.confirm("구매확정 이후에는 반품/교환이 불가능하므로 상품을 받으신 후 진행해주세요. 구매확정처리를 하시겠습니까?")) {
			return;
		}

		const res = await window.omsShopBridge.patch(`/customer/v1/orders/${orderCode}/sections/purchase-confirmation`, {
			orderSectionCode
		});
		if (typeof res['data'] !== "undefined" && res['data']['updateResult'] === true) {
			window.location.reload();
		}
   ...
   */
	const omsWithdrawReturnOrder = function({qty=0,orderCode,orderSectionCode,orderSectionItemCode,flag=false}) {
		if (typeof window.omsShopBridge === 'undefined') return;
		...
		if(confirm(getLocalizeString(localizeCode, '', defaultString))){
			...
			window.omsShopBridge.patch(`/customer/v1/orders/${orderCode}/sections/revert-return-request`,requestData).then((res)=>{
				if(res?.data?.msg !== "SUCCESS"){return alert('일시적인 에러입니다. 잠시 후 다시 시도해주세요.')}
				if(!res?.data?.data?.updateResult){return alert('반품 철회에 실패하였습니다. 관리자에게 문의해주세요.')}
				window.location.reload();
			})
		}
	}
	...
	const omsWithdrawCancelOrder = function({qty=0,orderCode,orderSectionCode,orderSectionItemCode}) {
		...
		if(confirm(getLocalizeString('타이틀_취소신청을철회하시겠습니까', '', '취소신청을 철회 하시겠습니까?'))){
			...
			window.omsShopBridge.patch(`/customer/v1/orders/${orderCode}/sections/revert-cancel-request`,requestData).then((res)=>{
				if(res?.data?.msg !== "SUCCESS"){return alert('일시적인 에러입니다. 잠시 후 다시 시도해주세요.')}
				if(!res?.data?.data?.updateResult){return alert('취소 철회에 실패하였습니다. 관리자에게 문의해주세요.')}
				window.location.reload();
			})
		}
	}
