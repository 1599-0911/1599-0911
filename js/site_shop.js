...
var SITE_SHOP_DETAIL = function () {
    ...
    */
     ...
     */
    var addOrder = function (type, backurl, is_c3, callback){
        ...
        $.ajax({
            ...
            data: _data,
            // C3 path 분기
			url : is_c3 ? '/shop/oms/OMS_add_order.cm' : '/shop/add_order.cm',
            dataType: 'json',
            cache: false,
            success: function (result) {
                if (result.msg == 'SUCCESS') {
    ...
    var showPCOptions = function (type, backurl = location.href, is_c3 = false){
        ...
        if ($prod_detail.hasClass('open')) {
            if(type == 'cart'){
                ...
                if (is_c3) {
                    SITE_SHOP_DETAIL.C3_addOrder('normal', backurl);
                }else{
	    ...
	    if(options.shop_view_tab_display !== "Y" || review_paging || qna_paging){
		    switch (true) {
			    ...
			    case type === SHOP_CONST.TAB_TYPE.REVIEW: {
				    ...
				    $.ajax({
					    type: 'POST',
					    data: { 'prod_idx': current_prod_idx, 'review_page': review_page, 'qna_page': qna_page, 'only_photo': only_photo, 'is_mobile' : SHOP_CONST.IS_MOBILE },
					    url: ('/shop/prod_review_pc_html.cm'),
					    dataType: 'html',
					    cache: false,
					    async: false,
		    ...
		    switch (true) {
			    ...
			    case type === SHOP_CONST.TAB_TYPE.REVIEW: {
				    $.ajax({
					    ...
					    data: { 'prod_idx': current_prod_idx, 'review_page': review_page, 'qna_page': qna_page, 'only_photo': only_photo, 'member_hash': MEMBER_HASH, 'is_mobile' : SHOP_CONST.IS_MOBILE },
					    url: ('/shop/prod_review_mobile_html.cm'),
					    dataType: 'html',
					    async: false,
					    success: function (result) {
    ...
    return {
        ...
        addOrder: function (type, backurl, params) {
            ...
            addOrder(type, backurl, false, function(result){
                switch (type) {
                    case 'npay':
                        if (result.npay_url == '') {
                            if (result.errmsg) {
        ...
        // C3 메서드 추가
		C3_addOrder : function(type, backurl, params){
			if(add_order_progress_check) return false;
			if ( typeof AM_PRODUCT != 'undefined' ) AM_PRODUCT(order_count);
			...
			addOrder(type, backurl, true, function(result){
				switch( type ) {
					case 'npay':
						if(result.npay_url == '') {
							if ( result.errmsg ) {
		...
		C3_showPCOptions : function(type, backurl){
...
var SITE_SHOP_CART = function () {
     ...
     */
    var addOrderWithCart = function (type, item_code, backurl, direct, is_c3, params){
        ...
        $.ajax({
            type: 'POST',
            data: _data,
			url : is_c3 ? '/shop/oms/OMS_add_order_cart.cm' : "/shop/add_order_cart.cm",
            dataType: 'json',
            cache: false,
            success: function (result) {
                if (result.msg == 'SUCCESS') {
    ...
    return {
		...
		C3_addOrderWithCartDirect : function(type, item_code, backurl){
			addOrderWithCart(type, item_code, backurl, true, true);
		},
		C3_addOrderWithCart : function(type, item_code, backurl, params){
